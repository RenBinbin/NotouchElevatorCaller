...
public class ScanInTheCallActivity extends BaseActivity implements CannyCallback,AMapLocationListener{
    ...
    protected int getLayoutId() {
        return R.layout.activity_scan_in_the_call;
    }
    ...
    private void ShowContent() {
        ...
        btnOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mWeiboDialog = WeiboDialogUtils.createLoadingDialog(ScanInTheCallActivity.this, "传输中...");
    ...
    public void onSuccess(String method, Object result, Object other) {
        if (method == "CallRequest") {
            CannyResponse cannyResponse= (CannyResponse) result;
            t.schedule(new TimerTask() {
                @Override
                public void run() {
                    CannyApi.PhoneStatus(SharedPrefOP.getInstance().getOpenId(),
                            SharedPrefOP.getInstance().getBianHao(), cannyResponse.msg.toString(), ScanInTheCallActivity.this);
                    index++;
                    if(index>15) {
                        t.cancel();
    ...
    Runnable runAbleUi=new Runnable(){
        ...
        public void run() {
            //更新界面
            Toast.makeText(ScanInTheCallActivity.this,"系统繁忙,请重试",Toast.LENGTH_SHORT).show();
            mWeiboDialog.dismiss();
        }
    };
